...
export default function App() {
  ...
  const prev = {
    ...
    bodyOvX: body.style.overflowX,
    rootBg: root?.style.backgroundColor,
    rootW: root?.style.width,
    rootMinW: root?.style.minWidth,
  };

  html.style.backgroundColor = '#ffffff';
  body.style.backgroundColor = '#ffffff';
  body.style.margin = '0';           // kill the default 8px body margin
  body.style.overflowX = 'hidden';   // avoid 1px slivers on ultrawide
  ...
  return () => {
    ...
    body.style.overflowX = prev.bodyOvX || '';
    if (root) {
      root.style.backgroundColor = prev.rootBg || '';
      root.style.width = prev.rootW || '';
      root.style.minWidth = prev.rootMinW || '';
    }
  };
}, []);
